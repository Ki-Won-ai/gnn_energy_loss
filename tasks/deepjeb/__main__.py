"""
CLI entry point for DeepJEB task.
...
Usage:
    ...
    uv run python -m tasks.deepjeb predict --model-checkpoint-path outputs/best.ckpt
"""
...
def prepare(config: str, overrides: tuple):
    ...
    Examples:
        uv run python -m tasks.deepjeb prepare
        uv run python -m tasks.deepjeb prepare -o num_workers=16
    """
...
def train(config: str, overrides: tuple):
    """
    Train model on DeepJEB benchmark.

    Examples:
        uv run python -m tasks.deepjeb train
        uv run python -m tasks.deepjeb train --config train-mini
        uv run python -m tasks.deepjeb train -o model=gcn
...
def predict(config: str, overrides: tuple, model_checkpoint_path: str):
    ...
    Examples:
        ...
        uv run python -m tasks.deepjeb predict -m outputs/best.ckpt -o visualization.enabled=false
        uv run python -m tasks.deepjeb predict -m outputs/best.ckpt -o output_dir=outputs/my-predictions
    """
...
def visualize(config: str, overrides: tuple, results_path: str):
    ...
    Examples:
        ...
        uv run python -m tasks.deepjeb visualize -r outputs/predictions/results.pt -o visualization.vmin=-2
        uv run python -m tasks.deepjeb visualize -r outputs/predictions/results.pt -o visualization.error_vmax=1.0
    """
